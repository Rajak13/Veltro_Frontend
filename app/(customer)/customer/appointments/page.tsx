...
export default function AppointmentsPage() {
  ...
  const allAppointments = (appointments ?? []) as RichAppointment[];
  const totalPages = Math.ceil(allAppointments.length / PAGE_SIZE);
  const paged = useMemo(
    () => allAppointments.slice((page - 1) * PAGE_SIZE, page * PAGE_SIZE),
  ...
  return (
    <div>
              ...
              <table className="w-full border-collapse">
                <thead>
                  <tr className="border-b border-zinc-100">
                    {["Date & Time", "Vehicle", "Service Type", "Status", "Actions"].map(h => (
                      <th key={h} className="px-5 py-3 text-left text-[11px] font-semibold text-zinc-400 uppercase tracking-wider">
                        {h}
                      </th>
                    ))}
                  </tr>
                      ...
                      <tr key={id} className="border-b border-zinc-50 hover:bg-zinc-50/60 transition-colors last:border-b-0">
                        ...
                        <td className="px-5 py-3.5">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-[13px] text-zinc-700 font-medium">{vehicleLabel}</span>
                          </div>
                        </td>
                        <td className="px-5 py-3.5">
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title="Book Appointment" size="lg">
        <BookAppointmentForm onSuccess={() => { setModalOpen(false); setPage(1); }} />
      </Modal>
    </div>
  );
}
