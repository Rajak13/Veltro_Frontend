...
export default function PartRequestsPage() {
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      case "Sourced":    return <Check className="w-5 h-5 text-green-600" />;
      case "Unavailable": return <X className="w-5 h-5 text-red-600" />;
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case "Sourced":    return "bg-green-50";
      case "Unavailable": return "bg-red-50";
  ...
  return (
    <div>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title="Request a Part">
        <CreatePartRequestForm onSuccess={() => { setModalOpen(false); setPage(1); }} />
      </Modal>
    </div>
  );
}
