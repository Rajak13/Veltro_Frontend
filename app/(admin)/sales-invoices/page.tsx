...
// Sales Invoices — Admin view (same functionality as staff, under admin layout)
...
import { Plus, Tag, Mail, Download } from "lucide-react";
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle="View and manage all customer sales"
        breadcrumb={[{ label: "Admin" }, { label: "Sales Invoices" }]}
        action={
          ...
          columns={[
            ...
            },
            {
              ...
              render: (r) => {
                ...
                return <Badge label={paid ? "Paid" : "Unpaid"} variant={paid ? "success" : "warning"} />;
              },
            },
            {
              key: "saleDate",
            ...
            },
            {
              ...
              header: "Actions",
              render: (r) => (
                <Button variant="outline" size="sm" onClick={() => setSelectedInvoice(r as unknown as SalesInvoice)}>
                  View
                </Button>
              ),
            },
      ...
      <Modal open={createOpen} onClose={() => setCreateOpen(false)} title="New Sales Invoice" size="xl">
      ...
      >
        {selectedInvoice && (
          <InvoiceDetail invoice={selectedInvoice as unknown as Record<string, unknown>} onClose={() => setSelectedInvoice(null)} />
        )}
      </Modal>
    </div>
  );
...
function InvoiceDetail({ invoice, onClose }: { invoice: Record<string, unknown>; onClose: () => void }) {
    ...
    <div className="space-y-5 text-sm">
      <div className="grid grid-cols-2 gap-4">
        ...
        <div>
          ...
          <p className="text-zinc-700">{new Date(String(invoice.saleDate ?? invoice.createdAt ?? "")).toLocaleString()}</p>
        </div>
        <div>
              ...
              <tbody>
                {items.map((item, i) => (
                  <tr key={i} className="border-b border-zinc-50 last:border-0">
                    ...
                    <td className="px-3 py-2 text-right tabular-nums text-zinc-600">Rs. {Number(item.unitPrice ?? 0).toLocaleString()}</td>
      ...
      <div className="rounded-xl border border-zinc-200 bg-zinc-50 p-4 space-y-2">
        ...
        {discount > 0 && (
          <div className="flex justify-between text-green-600">
            <span className="flex items-center gap-1.5"><Tag className="w-3.5 h-3.5" />Loyalty discount (10%)</span>
      ...
      <div className="flex justify-end gap-3 pt-1 border-t border-zinc-100">
        ...
        />
        <Button size="sm" loading={isSending} onClick={() => sendEmail(invoiceId)} disabled={!invoiceId}>
