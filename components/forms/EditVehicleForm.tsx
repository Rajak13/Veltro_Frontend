...
import { Car, Wrench, Calendar, Hash, Gauge, Upload, X } from "lucide-react";
...
export default function EditVehicleForm({ vehicle, onSuccess }: Props) {
  ...
  const onSubmit = async (data: FormData) => {
    try {
      await mutateAsync({
        id: vehicle.vehicleId as unknown as number,
        ...data,
      ...
      toast.error(msg ?? "Failed to update vehicle");
    }
  };
    ...
    <form onSubmit={handleSubmit(onSubmit)} noValidate className="space-y-4">
      ...
      <div className="flex flex-col gap-1">
        <label className="text-[11px] font-medium text-zinc-600">
          Vehicle Photo <span className="text-zinc-400">(optional)</span>
        </label>
        {imagePreview ? (
          <div className="relative rounded-xl overflow-hidden border border-zinc-200 bg-zinc-50">
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src={imagePreview} alt="Vehicle" className="w-full h-40 object-cover" />
            <button
              type="button"
      ...
      {/* Mileage */}
      <Field label="Mileage (km)" icon={<Gauge className="w-3.5 h-3.5" />} error={errors.mileage?.message}>
        <input {...register("mileage")} type="number" placeholder="50000" className={inputCls} />
      </Field>
